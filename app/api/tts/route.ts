...
export const POST = withRoute(async (req: Request) => {
  ...
  // A refusal is not a failure, and the difference decides whether the caller
  // retries or gives up on this provider.
  //
  //   401 — the key is wrong, or (what actually happens here) it is a scoped key
  //         with `text_to_speech` switched off. It authenticates and is then
  //         refused, which is indistinguishable from "not configured".
  //   402 — the plan does not include API synthesis. The free tier returns this.
  //   403 — the key is real but not permitted to do this.
  //
  // None of the three is fixed by retrying and none is upstream being sad; each
  // is fixed by a person changing a setting in a dashboard. So they collapse
  // into `tts-unavailable`, the same code a missing key produces, which is the
  // signal the client falls back to the on-device voice on. A user must never
  // lose an answer because a voice UPGRADE is unavailable.
  if (upstream.status === 401 || upstream.status === 402 || upstream.status === 403) {
    console.error('[tts] provider refused synthesis', upstream.status);
    throw new ProblemError('tts-unavailable');
  }
