 ...
 * with a `mode` prop. The two screens post to the same endpoints today, but
 ...
 * later than two files are to keep in step now. The password rule below is the
 * first thing that only one of the two shows.
 */
...
type Status =
  ...
  | { k: 'signing-up' } // password in flight
...
export function SignUpForm() {
  ...
  const busy = status.k === 'sending' || status.k === 'signing-up';
  ...
  async function onSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (busy) return;
    setStatus({ k: 'signing-up' });

    try {
      await apiFetch<Me>('/auth/password', {
        method: 'POST',
        body: { email, password, intent: 'sign_up' },
      });
  ...
  // The rule is shown up front, so the password field is always described by
  // something; the error id joins it only when there is a message.
  const passwordDescribedBy = error !== null ? 'password-rule auth-error' : 'password-rule';
  ...
  return (
    <form onSubmit={onSubmit} noValidate>
      ...
      {/* The rule is stated before the first attempt, not after a rejected one.
          --text-secondary, not --text-tertiary: tertiary measures 2.81:1 and may
          not carry words a user has to read. */}
      <p
        id="password-rule"
        className="mt-[var(--space-7)] text-secondary"
        style={{ font: 'var(--type-caption)' }}
      >
        8자 이상
      </p>
      ...
      <p
        ...
        role="status"
        aria-live="polite"
        className="mt-[var(--space-5)] min-h-[var(--caption-lh)] text-secondary"
        style={{ font: 'var(--type-caption)' }}
      >
        {error?.message ?? ''}
      </p>
      ...
      >
        {status.k === 'signing-up' ? '가입하는 중…' : '가입하기'}
      </Button>
